...
const Comment = ({ navigation, commentData, first }) => {
  ...
  const commentStyle = first ? globalStyles.first_comment : globalStyles.comment;
  return (
    <SafeAreaView>
      <View style={commentStyle}>
        <UserPicName uid={commentData.uid} navigation={navigation} />
        {commentData.uid == currentUser.uid ? (
          <Entypo style={globalStyles.edit_comment} name="dots-three-horizontal" size={20}></Entypo>
        ) : null}
